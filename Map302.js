...
const saskRms = L.esri
    .featureLayer({
        apiKey: apiKey,
        url: "https://gis.saskatchewan.ca/arcgis/rest/services/Administrative/MapServer/4",
        simplifyFactor: 0.35,
        renderer: L.canvas(),
        precision: 5,
...
const albertaRms = L.esri
    .featureLayer({
        apiKey: apiKey,
        url: "https://geospatial.alberta.ca/titan/rest/services/boundary/urban_and_rural_municipality/MapServer/107",
        simplifyFactor: 0.35,
        renderer: L.canvas(),
        precision: 5,
        fetchAllFeatures: true,
...
const manitobaRms = L.esri
    .featureLayer({
        apiKey: apiKey,
        url: "https://agrimaps.gov.mb.ca/arcgis/rest/services/AGRIMAPS/AGRIMAPS/MapServer/7",
        simplifyFactor: 0.35,
        renderer: L.canvas(),
        precision: 5,
        fetchAllFeatures: true,
...
albertaRms.on("createfeature", function(e) {
    ...
    const label = L.marker(center, {
        icon: L.divIcon({
            iconSize: null,
            className: 'rmlabel',
            html: "<div>" + e.feature.properties.MD_NAME + "</div>"
        })
    });
    markers.addLayer(label);
});

manitobaRms.on("createfeature", function(e) {
    const id = e.feature.id;
    const feature = manitobaRms.getFeature(id);
    const center = feature.getBounds().getCenter();
    const label = L.marker(center, {
        icon: L.divIcon({
            iconSize: null,
            className: 'rmlabel',
            html: "<div>" + e.feature.properties.MUNI_NAME + "</div>"
        })
    });
    markers.addLayer(label);
});
