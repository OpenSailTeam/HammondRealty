        ...
        const observer = new MutationObserver(function(mutationsList, observer) {
            for(let mutation of mutationsList) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                    if (window.getComputedStyle(mapContainerElement).display === 'block') {
                        populateMap();
                        observer.disconnect();  // Stop observing once we've loaded the map
                    }
                }
            }
        });
...
function populateMap() {
    ...
    const mapContainerElement = document.getElementById('map-container');
    if (mapContainerElement && window.getComputedStyle(mapContainerElement).display !== 'block') {
        return;
    }
